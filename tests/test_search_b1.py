...
def test_txh_date_range_presets():
    erp.st.session_state.clear()
    today = erp.datetime.date.today()
    erp.st.session_state["txh_date_preset"] = "today"
    start, end = erp._txh_date_range()
    assert start == today and end == today

    erp.st.session_state["txh_date_preset"] = "month"
    start, end = erp._txh_date_range()
    assert start == today.replace(day=1) and end == today

    erp.st.session_state["txh_date_preset"] = "custom"
    erp.st.session_state["txh_date_from"] = today.replace(day=5)
    erp.st.session_state["txh_date_to"] = today.replace(day=1)
    start, end = erp._txh_date_range()
    assert start == today.replace(day=1) and end == today.replace(day=5)
...
def test_txh_inline_search_and_date_filters_wired():
    ...
    assert 'key=f"txh_date_preset_{preset}"' in src
...
def test_search_scope_i18n():
    ...
    assert t("search.scope.txn_history", "en") == "Searching Transaction History only"
