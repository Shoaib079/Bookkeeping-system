...
import type { PurchasesListResponse } from "../lib/api/types";
...
export function PurchasesPage() {
  ...
  const [page, setPage] = useState<PurchasesListResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await apiGet<PurchasesListResponse>("/api/v1/purchases", {
          session,
          companyScoped: true,
        });
        if (!cancelled) {
  ...
  return (
    <section className="erp-purchases-page">
      <header className="erp-page-header">
        <h1>Purchases</h1>
        <p className="erp-page-header__meta">
          Read-only purchase register via `/api/v1/purchases`
        </p>
      </header>
      ...
      {page ? (
        <>
          <div className="erp-ledger-summary">
            <span>Purchases: {page.row_count}</span>
          </div>
