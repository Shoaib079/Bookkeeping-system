...
- **AUTH-SESSION-01-IMPL-3 — optional "remember this device":** per-login toggle controlling cookie write + TTL (idle vs absolute split).
- **AUTH-SESSION-01-IMPL-4 — HttpOnly hardening (FastAPI migration):** move cookie-setting server-side and HttpOnly; unify with the JWT access+refresh model (`services/tokens.py`) for the React front end.
