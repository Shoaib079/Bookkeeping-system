...
|---|---|---|
| **Primary router** | `_PAGE_DISPATCH` dict → `render_*` per `nav_selection` — **derived from `registry/navigation.py`** | `26160`; dispatch call `26385` |
| **Desktop sidebar render** | `_render_navigation_tree(st.sidebar, …)` (custom button accordion) | def `3189`; call `26342` |
| **`st.sidebar` usage (only 2 sites)** | date-range filters; nav tree | `1008-1009`, `26343` |
...
| **Accordion groups** | `_NAV_ACCORDION` (8 groups) — **derived from registry** | `3181` |
| **Direct (top-level) pages** | `_NAV_DIRECT_PAGES` — **derived from registry** | `3183` |
| **Role visibility** | `_NAV_ROLE_PAGES` (owner/manager/cashier/partner/viewer) — **derived from registry** | `3186` |
| **Mobile bottom nav** | `_MOBILE_BOTTOM_NAV` (5 slots) — **derived from registry** | `3169` |
| **Mobile hubs** | `_MOBILE_HUB_CONFIG` (money/reports/people/more) — **derived from registry** | `3178` |
