...
- **MONO-THEME-01-S6 — reports / tables / banking statuses:** ✅ **Complete** — financial/statement tables, stTable, desktop/mobile txn ledger, status pills, and banking/report section chips route through `--erp-table-*` and chip extension tokens; `tests/test_mono_theme_01_s6_table_status_grammar.py`. Tag: `mono-theme-01-s6-table-status-grammar`. Semantic row/status colors unchanged.
- **MONO-THEME-01-S7 — cleanup + React contract update:** ✅ **Complete** — `auth.css` role chips use mono grammar (no `--role-*` references); `react_token_bundle()` exports `componentGrammar` + grammar key families; `tests/test_mono_theme_01_s7_react_contract_cleanup.py`. Tag: `mono-theme-01-s7-react-contract-cleanup`. Epic **MONO-THEME-01 complete**.
