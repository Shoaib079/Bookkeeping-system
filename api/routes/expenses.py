"""FASTAPI-P2.2 — expense write endpoint."""
...
from services.write_expenses import ExpenseWriteResult, create_and_post_expense
...
def _write_expenses_enabled() -> bool:
    return os.getenv(WRITE_EXPENSES_ENV, "").strip().lower() in ("1", "true", "yes")


def _require_write_expenses_feature() -> None:
    if not _write_expenses_enabled():
        raise HTTPException(status_code=404, detail="Not Found")


def _result_to_response(result: ExpenseWriteResult) -> CreateExpenseResponse:
...
@router.post(
    "",
    status_code=201,
    summary="Create and post an expense",
    description=(
        "Records a cash or bank expense and posts the GL entry. "
        "Requires ``ERP_API_WRITE_EXPENSES=1``. Company scope comes from "
...
def post_expense(
    ...
    _require_write_expenses_feature()
    ...
    try:
        result = create_and_post_expense(
            session,
